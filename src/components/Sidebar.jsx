const navGroups = [
  {
    ...
    items: [
      ...
      { id: 'gst', label: 'GST Dashboard', icon: 'chart' },
    ],
  },
  {
    label: 'Documents',
...
const Icon = ({ name, className = 'w-[18px] h-[18px]' }) => {
  const paths = {
    ...
    chart: <path d="M3 17V7m4 10v-6m4 6V4m4 13v-8m4 8V10" />,
...
export default function Sidebar({ view, onNav, company, userEmail, onSignOut, mobileOpen, onCloseMobile }) {
  ...
  return (
    <>
      {mobileOpen && <div className="md:hidden fixed inset-0 bg-ink/30 z-30 no-print" onClick={onCloseMobile} />}
        ...
        <div className="h-full flex flex-col">
          <div className="px-5 py-5 border-b hairline">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col leading-none">
                <span className="font-display font-bold text-base tracking-tightest">AllyTechSoft</span>
                <span className="font-mono text-[9px] tracking-[0.25em] uppercase text-mute mt-0.5">Invoice</span>
              </div>
            </div>
          </div>
          ...
          <nav className="flex-1 px-3 py-3">
            {navGroups.map((g, gi) => (
              <div key={gi} className={gi > 0 ? 'mt-4' : ''}>
                {g.label && (
                  <div className="px-3 mb-1 text-[10px] uppercase tracking-[0.18em] text-mute font-mono">
                    {g.label}
                  </div>
                )}
                      ...
                      <li key={item.id}>
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors
                                      ${active ? 'bg-brandBlue/10 text-brandBlue' : 'text-ash hover:bg-ice hover:text-ink'}`}
                        >
                          <Icon name={item.icon} />
                          {item.label}
                        </button>
          ...
          <div className="px-3 py-4 border-t hairline">
            <div className="px-2 mb-2">
              <div className="text-[10px] uppercase tracking-[0.2em] text-mute font-mono">Signed in as</div>
              <div className="text-sm font-medium text-ink truncate" title={userEmail}>{userEmail || '—'}</div>
              {company?.name && <div className="text-xs text-mute truncate mt-0.5">{company.name}</div>}
            </div>
            <button
              onClick={signOut}
